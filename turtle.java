 ...
 *       Turtle.render();
 *
 *  Many graphics packages use the convention that the (0,0)
 *  pixel is in the upper left corner, but Turtle is constructed
 *  so that that pixel in is the lower left corner, to make it
 *  correspond to the usual coordinate system of the plane. Thus,
 *  increasing the first index will move to the right in the grid
 *  and increasing the second coordinate will move higher in the
 *  grid.
 *  
 * Methods available
 * 
 * accessor methods for the Turtle's pen
 ...
 * movement and drawing methods 
       ...
       go(double x, double y)           Move to position x,y drawing during the movement
